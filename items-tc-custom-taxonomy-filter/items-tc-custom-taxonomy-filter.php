...
if ( is_admin() ) {
    add_action( 'restrict_manage_posts', 'items_tc_ctf_restrict_manage_posts' );
    add_filter( 'parse_query', 'items_tc_ctf_convert_taxonomy_id_to_slug' );
}

// Create our drop-down category listing, using all the latest & greatest WordPress things
function items_tc_ctf_restrict_manage_posts() {

    global $wp_query;

    $filters = items_tc_ctf_get_filters();
...
function items_tc_ctf_get_filters() {
    global $typenow;
...
function items_tc_ctf_convert_taxonomy_id_to_slug( $query ) {
    global $pagenow;
